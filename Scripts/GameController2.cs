...
{
    public static GameController_2 Instance;

    [SerializeField] private GameObject[] Lanes_3Prefabs;
    ...
    void Start()
    {
        ...
        {
            int n = Random.Range(0, Lanes_3Prefabs.Length);
            LanesOfStage[i] = Instantiate(Lanes_3Prefabs[n]);
        }

        LaneSize = LanesOfStage[0].GetComponent<Transform>().Find("Lane_1").localScale.z; // Récupère la scale Z du Lane

        float pos = Sphere.transform.position.x + LaneSize / 2 - 1.5f;
